...
function validate( opts, options ) {
	if ( !isObject( options ) ) {
		return new TypeError( format( '1Tt2V,FD', options ) );
	}
	if ( hasOwnProp( options, 'depth' ) ) {
		opts.depth = options.depth;
		if ( !isNonNegativeInteger( opts.depth ) ) {
			return new TypeError( format( '1Tt2t,G9', 'depth', opts.depth ) );
		}
	}
	if ( hasOwnProp( options, 'copy' ) ) {
		opts.copy = options.copy;
		if ( !isBoolean( opts.copy ) ) {
			return new TypeError( format( '1Tt2o,GE', 'copy', opts.copy ) );
		}
	}
	if ( hasOwnProp( options, 'flattenArrays' ) ) {
		opts.flattenArrays = options.flattenArrays;
		if ( !isBoolean( opts.flattenArrays ) ) {
			return new TypeError( format( '1Tt2o,GE', 'flattenArrays', opts.flattenArrays ) );
		}
	}
	if ( hasOwnProp( options, 'delimiter' ) ) {
		opts.delimiter = options.delimiter;
		if ( !isString( opts.delimiter ) ) {
			return new TypeError( format( '1Tt2W,Gh', 'delimiter', opts.delimiter ) );
		}
	}
	return null;
}
